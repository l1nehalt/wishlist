...
public class WishItemsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetById(Guid id)
    {
        ...
        return result != null
            ? Ok(new WishItemResponse(result.Id, result.Title, result.Description, result.Price, result.Link))
            : NotFound();
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody]CreateWishItemRequest request)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody]UpdateWishItemRequest request)
    {
    ...
    {
        await _wishItemsService.Delete(id);
